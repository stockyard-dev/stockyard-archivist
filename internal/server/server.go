package server
import("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-archivist/internal/store")
...
func(s *Server)routes(){
    s.mux.HandleFunc("GET /health",func(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-archivist"})})
    s.mux.HandleFunc("GET /api/stats",s.handleOverview)
    s.mux.HandleFunc("GET /api/documents",s.handleList)
    s.mux.HandleFunc("POST /api/documents",s.handleCreate)
    s.mux.HandleFunc("DELETE /api/documents/{id}",s.handleDelete)
